...
Orbsy is a fork of [Orbkit](https://github.com/zzzzshawn/orbkit) by
[zzzzshawn](https://x.com/zzzzshawn/): the whole kit (all 33 orbs, the gallery, the
details drawer, the DialKit playground, the docs and the agent/API surface) plus ten
new Orbsy orbs.
...
The Orbsy orbs live in `orbs/orbs/orbsy-*.tsx` and share GLSL helpers from
`orbs/orbs/orbsy-glsl.ts`, which the registry build ships alongside any orb that
imports it. They are original work under MIT. Identity (name, namespace, repo) is set in
`lib/site-config.ts`; set `REGISTRY_HOMEPAGE` to the deployed URL.

```bash
pnpm install
pnpm dev     # gallery at /, playground at /playground
pnpm build   # check:orbs, registry:build, next build
```

The upstream Orbkit README follows.
