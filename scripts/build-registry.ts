...
const importRewrites: ReadonlyArray<{ from: string; to: string }> = [
  ...
  { from: "./orbsy-glsl", to: "@/components/ui/orbsy-glsl" }
];

/**
 * GLSL helpers shared by the Orbsy orbs (orbsy-01 to orbsy-10). Shipped
 * alongside any orb that imports it, typed `registry:ui` for the same routing
 * reason as the core.
 */
const ORBSY_GLSL_TARGET_PATH = "components/ui/orbsy-glsl.ts";
...
async function build() {
  ...
  const orbsyGlslSource = await readFile(path.join(orbsRoot, "orbs", "orbsy-glsl.ts"), "utf-8");
  const orbsyGlslFile: RegistryFile = {
    path: ORBSY_GLSL_TARGET_PATH,
    type: "registry:ui",
    content: orbsyGlslSource
  };
  await writeRegistrySource(ORBSY_GLSL_TARGET_PATH, orbsyGlslSource);
  ...
  const orbRegistryItems = await Promise.all(
    orbRegistry.map(async (orb) => {
      ...
      const files: RegistryFile[] = [
        coreFile,
        ...(componentSource.includes(`"@/components/ui/orbsy-glsl"`) ? [orbsyGlslFile] : []),
